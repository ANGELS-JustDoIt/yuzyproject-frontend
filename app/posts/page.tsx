                  ...
                  <div className="flex items-center justify-between p-4">
                    <div className="flex items-center gap-3">
                      <div className="w-10 h-10 rounded-full bg-gradient-to-br from-[#339989] to-[#7DE2D1] flex items-center justify-center overflow-hidden">
                        {(post.profileImageUrl || post.profile_image_url) && !imageErrors.has(post.boardId) ? (
                          <img
                            src={(post.profileImageUrl || post.profile_image_url)?.startsWith('http') 
                              ? (post.profileImageUrl || post.profile_image_url) 
                              : `${API_BASE_URL}${post.profileImageUrl || post.profile_image_url}`}
                            alt="프로필 사진"
                            className="w-full h-full object-cover"
                            onError={() => {
                              setImageErrors((prev) => new Set(prev).add(post.boardId));
                            }}
                          />
                        ) : (
                ...
                <div>
                  ...
                  <div className="flex items-center gap-4 text-sm text-slate-400">
                    <div className="w-8 h-8 rounded-full bg-gradient-to-br from-[#339989] to-[#7DE2D1] flex items-center justify-center overflow-hidden flex-shrink-0">
                      {(selectedPost.profileImageUrl || selectedPost.profile_image_url) && !imageErrors.has(selectedPost.boardId) ? (
                        <img
                          src={(selectedPost.profileImageUrl || selectedPost.profile_image_url)?.startsWith('http') 
                            ? (selectedPost.profileImageUrl || selectedPost.profile_image_url) 
                            : `${API_BASE_URL}${selectedPost.profileImageUrl || selectedPost.profile_image_url}`}
                          alt="프로필 사진"
                          className="w-full h-full object-cover"
                          onError={() => {
                            setImageErrors((prev) => new Set(prev).add(selectedPost.boardId));
                          }}
                        />
                      ) : (
